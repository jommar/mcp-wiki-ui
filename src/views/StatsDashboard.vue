  ...
  <div class="max-w-5xl mx-auto px-6 py-8">
    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        ...
        <StatCard
          label="With Content"
          :value="stats.sectionsWithContent"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          <h3
            class="flex items-center gap-1.5 text-[13px] font-semibold text-heading uppercase tracking-wider mb-3"
          >
            <Eye class="w-3.5 h-3.5" /> Most Accessed
          </h3>
          <div class="space-y-0.5">
            ...
            >
              <span
                class="w-6 h-6 rounded-full bg-elevated border border-border text-[11px] text-muted font-mono flex items-center justify-center flex-shrink-0"
              >
                {{ i + 1 }}
              </span>
              ...
              <span class="text-[12px] text-accent font-mono font-semibold flex-shrink-0"
                >{{ s.accessCount }}×</span
              >
            </button>
            <p v-if="!stats.topAccessed?.length" class="text-[13px] text-muted italic py-3 px-3">
              No access data yet
            </p>
          </div>
        </div>
        ...
        <div>
          <h3
            class="flex items-center gap-1.5 text-[13px] font-semibold text-heading uppercase tracking-wider mb-3"
          >
            <TrendingUp class="w-3.5 h-3.5" /> Most Linked
          </h3>
          <div class="space-y-0.5">
            <button
            ...
            >
              ...
              <span class="text-[12px] text-accent font-mono font-semibold flex-shrink-0"
                >{{ s.linkCount }} links</span
              >
            </button>
            <p v-if="!stats.topLinked?.length" class="text-[13px] text-muted italic py-3 px-3">
              No link data yet
            </p>
          </div>
        </div>
      ...
      <div v-if="stats.recentlyEdited?.length" class="mt-8">
        <h3
          class="flex items-center gap-1.5 text-[13px] font-semibold text-heading uppercase tracking-wider mb-3"
        >
          <Clock class="w-3.5 h-3.5" /> Recently Edited
        </h3>
        <div class="space-y-0.5">
          <button
          ...
          >
            ...
            <span v-if="s.changeReason" class="text-[12px] text-muted italic truncate flex-1">{{
              s.changeReason
            }}</span>
            <span class="text-[11px] text-muted flex-shrink-0 ml-auto">{{
              fmtDate(s.changedAt)
            }}</span>
          </button>
        </div>
